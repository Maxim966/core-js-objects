...
function shallowCopy(obj) {
  return Object.assign(typeof obj === 'object' ? {} : [], obj);
}
...
function compareObjects(obj1, obj2) {
  ...
  return Object.keys(obj1).every(
    (key) => !obj2Keys.includes(key) || obj1[key] === obj2[key]
  );
}
...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([key, value]) => {
    const pairs = value.map((index) => [key, index]);
    arr1.push(...pairs);
  });
...
function sellTickets(queue) {
  let cash = 0;
  const [banknote25, banknote50, banknote100] = [25, 50, 100];

  for (let index = 0; index < queue.length; index += 1) {
    if (queue[index] === banknote25) {
      cash += banknote25;
    }

    if (queue[index] === banknote50) {
      if (cash === 0) {
        return false;
      }
      cash = cash + banknote50 - banknote25;
    }

    if (queue[index] === banknote100) {
      if (cash === 0) {
        return false;
      }
...
function sortCitiesArray(arr) {
  return [...arr].sort((a, b) => {
    const countries = Object.values(a)[0].localeCompare(Object.values(b)[0]);
    const city = Object.values(a)[1].localeCompare(Object.values(b)[1]);
    if (countries === 0) {
      return city;
    }
    return countries;
